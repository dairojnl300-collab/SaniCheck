const Verificar = (() => {
  ...
  async function revisarCliente(fotoId, estado) { const inspeccion = Store.getCurrentInspeccion(); const aspectos = inspeccion?.programas.flatMap(p => p.aspectos.flatMap(a => [a, ...(a.criterios_extra || []).map((x, i) => { if (!x.id) x.id = `${a.id}-extra-${i + 1}`; return x; })])); const foto = aspectos?.flatMap(a => a.fotografias || []).find(f => f.id === fotoId); if (!foto || !inspeccion.portal_informe_id || !foto.aspecto_id) return; const observacion = estado === 'ajustes_solicitados' ? prompt('Indica qué debe corregirse:') : null; if (estado === 'ajustes_solicitados' && !observacion?.trim()) return; try { await ScInformes.revisarAdminHallazgo(inspeccion.portal_informe_id, foto.aspecto_id, estado, observacion); foto.estado_portal = estado === 'cumple' ? 'Verificado' : 'En corrección'; if (estado === 'cumple') { const aspecto = aspectos.find(a => a.id === foto.aspecto_id); if (aspecto) { aspecto.evaluacion = 'A'; aspecto.criterio = 'A'; aspecto.estado = 'Cerrado'; } } Scores.calcular(inspeccion); Hallazgos.actualizar(inspeccion); Store.upsertInspeccion(inspeccion); Router.toast(estado === 'cumple' ? 'Revisado y verificado' : 'Ajustes solicitados'); _refresh(); } catch (e) { Router.toast(e.message || 'No se pudo revisar la evidencia'); } }
