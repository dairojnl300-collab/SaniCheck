const Verificar = (() => {
  ...
  async function revisarCliente(fotoId, estado) { const inspeccion = Store.getCurrentInspeccion(); const estadoRevision = estado === 'cumple' ? 'cumple' : estado === 'ajustes_solicitados' ? 'ajustes_solicitados' : null; if (!estadoRevision) return Router.toast('Acción de revisión inválida'); const aspectos = inspeccion?.programas.flatMap(p => p.aspectos.flatMap(a => [a, ...(a.criterios_extra || []).map((x, i) => { Hallazgos.idExtra(a, x, i); return x; })])); const registro = aspectos?.flatMap(aspecto => (aspecto.fotografias || []).map(foto => ({ foto, aspecto }))).find(x => x.foto.id === fotoId); const foto = registro?.foto; const aspecto = registro?.aspecto; if (!foto || !aspecto) return Router.toast('No se encontró el aspecto de esta evidencia'); if (!inspeccion.portal_informe_id && ScInformes.sincronizarHallazgosPortal) { Router.toast('Vinculando informe…'); await ScInformes.sincronizarHallazgosPortal(inspeccion); } if (!inspeccion.portal_informe_id) return Router.toast('No se pudo vincular el informe. Intenta nuevamente.'); let aspectoId = aspecto.id; if (ScInformes.getInforme && foto.path) { try { const remoto = await ScInformes.getInforme(inspeccion.portal_informe_id); const candidatos = (remoto?.estado_estructurado?.inspeccion?.hallazgos_criticos || []).filter(h => h.foto_url === foto.path || h.foto_path === foto.path); const mismoAspecto = candidatos.find(h => h.aspecto_id === aspectoId); if (mismoAspecto?.aspecto_id) aspectoId = mismoAspecto.aspecto_id; else if (candidatos.length === 1 && candidatos[0].aspecto_id) aspectoId = candidatos[0].aspecto_id; } catch (e) { /* se conserva el ID local si el informe remoto no responde */ } } if (!aspectoId) return Router.toast('No se encontró el identificador del aspecto'); foto.aspecto_id = aspectoId; const observacion = estadoRevision === 'ajustes_solicitados' ? prompt('Indica qué debe corregirse:') : null; if (estadoRevision === 'ajustes_solicitados' && !observacion?.trim()) return; try { await ScInformes.revisarAdminHallazgo(inspeccion.portal_informe_id, aspectoId, estadoRevision, observacion); foto.estado_portal = estadoRevision === 'cumple' ? 'Verificado' : 'En corrección'; if (estadoRevision === 'cumple') { aspecto.evaluacion = 'A'; aspecto.criterio = 'A'; aspecto.estado = 'Cerrado'; } Scores.calcular(inspeccion); Hallazgos.actualizar(inspeccion); Store.upsertInspeccion(inspeccion); Router.toast(estadoRevision === 'cumple' ? 'Revisado y verificado' : 'Ajustes solicitados'); _refresh(); } catch (e) { console.error('[Verificar] revisión de cliente falló', { fotoId, aspectoId, estado: estadoRevision, error: e }); Router.toast(e.message || 'No se pudo revisar la evidencia'); } }
