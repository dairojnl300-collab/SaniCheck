const Verificar = (() => {
  ...
  function _detalle(x) {
    ...
    return `<article class="dash-detail" style="display:block;padding:10px 11px;margin-bottom:8px;border:1px solid var(--line);border-left:3px solid ${color};border-radius:var(--radius-sm);background:#fff;">
      ...
      ${(x.fotografias || []).filter(f => f.eliminada !== true).map(f => `<div style="display:inline-block;vertical-align:top;margin:6px 6px 0 0;"><button class="dash-photo" onclick="Verificar.mostrarFoto('${f.id}')"><img loading="lazy" decoding="async" ${f.data ? `src="${f.data}"` : ''}${f.path ? `data-foto-path="${_esc(f.path)}"` : ''} alt="Foto del aspecto"></button>${f.origen === 'Cliente' ? `<small style="display:block;color:var(--ink-55);margin-top:4px;">Foto subida por el cliente${f.tomada_en ? ` · ${_esc(new Date(f.tomada_en).toLocaleString('es-CO'))}` : ''}</small>${f.superada === true ? '<small style="display:block;color:var(--ink-55);margin-top:4px;">Evidencia anterior</small>' : /^verificado$|^cumple$/i.test(String(f.estado_portal || '')) ? '<strong style="display:block;color:#2E7D32;margin-top:6px;">✓ Revisado y verificado</strong>' : `<div style="display:flex;gap:5px;margin-top:5px;flex-wrap:wrap;"><button type="button" style="${_btnStyle('#2E7D32','#fff')}" onclick="event.stopPropagation();Verificar.revisarCliente('${_esc(f.id)}','cumple','${_esc(x.id)}')">Marcar como cumple</button><button type="button" style="${_btnStyle('#B45309','#fff')}" onclick="event.stopPropagation();Verificar.revisarCliente('${_esc(f.id)}','ajustes_solicitados','${_esc(x.id)}')">Solicitar ajustes</button></div>`}` : ''}</div>`).join('')}
    </article>`;
  }
