  ...
  function _renderDetallePorItem(inspeccion, options = {}) {
    ...
    const tarjeta = (a, numero, criterioTitulo) => {
      ...
      return `<div class="acta-card" data-aspecto-id="${_esc(aspectoId)}" data-aspecto-numero="${_esc(numero)}" style="padding:9px 10px;border:1px solid #E5E7EB;border-left:3px solid ${c};border-radius:6px;background:#fff;break-inside:avoid;page-break-inside:avoid;">
        ...
        ${cumple ? `<div data-linea-accion style="margin-top:7px;font-size:10px;color:#374151;text-align:justify;hyphens:auto;"><strong data-etiqueta-accion>Observaciones:</strong> ${_esc(a.obs || 'Sin observaciones registradas.')}</div><div style="margin-top:4px;padding:5px 7px;background:#F0FAF5;border-radius:4px;font-size:10px;color:#374151;text-align:justify;hyphens:auto;"><strong>Recomendaciones:</strong> ${_esc(a.recomendaciones || 'Sin recomendación registrada.')}</div><div data-verificado-ecodesa hidden></div>` : ''}
    ...
    const detalleDesktop = conEval.map(({ p, programaIdx }) => {
      const criterios = p.aspectos.map((base, criterioIdx) => ({ base, criterioIdx, aspectos: [{ ...base, aspecto_id: base.aspecto_id || base.id, extraIdx: null }, ...(base.criterios_extra || []).map((x, i) => ({ ...x, id: x.id || `${base.id}-extra-${i + 1}`, aspecto_id: x.aspecto_id || `${base.id}-extra-${i + 1}`, norma: base.norma, fotografias: x.fotografias || [], extraIdx: i }))].filter(x => x.evaluacion || x.criterio || (x.fotografias || []).length) })).filter(x => x.aspectos.length);
