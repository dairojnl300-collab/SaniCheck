const Hacer = (() => {
  ...
  function _programarBorrador(s, options = {}) {
    if (!s || typeof ScInformes === 'undefined' || typeof ScInformes.scheduleBorrador !== 'function') return;
    ScInformes.scheduleBorrador(s.inspeccion, {
      ...options,
      cursor: options.cursor || {
        programa_id: s.programa.id,
        aspecto_id: s.aspecto.id,
        criterio: 'base',
      },
    });
  }
  ...
  function evaluar(valor) { const s = _state(); if (!s) return; s.aspecto.criterio = valor; s.aspecto.evaluacion = valor; if (valor === 'A') { s.aspecto.estado = 'Cerrado'; s.aspecto.hallazgo = ''; s.aspecto.accion = ''; } else if (valor === 'I') { s.aspecto.estado = 'Abierto'; } else { s.aspecto.estado = 'Cerrado'; } Scores.calcular(s.inspeccion); Hallazgos.actualizar(s.inspeccion); Store.upsertInspeccion(s.inspeccion); _programarBorrador(s, { force: true, aspectKey: `${s.programa.id}:${s.aspecto.id}:base` }); _refresh(); }
  function guardar(campo, valor) { const s = _state(); if (!s) return; s.aspecto[campo] = valor; if (campo === 'hallazgo' || campo === 'obs') { if (campo === 'hallazgo') s.aspecto.obs = valor; s.aspecto.obs_editada = true; } Hallazgos.actualizar(s.inspeccion); Store.upsertInspeccion(s.inspeccion); _programarBorrador(s, { aspectKey: `${s.programa.id}:${s.aspecto.id}:base` }); }
  ...
  function eliminarCriterio(index) {
    ...
    Scores.calcular(s.inspeccion); Hallazgos.actualizar(s.inspeccion); Store.upsertInspeccion(s.inspeccion); _programarBorrador(s, { aspectKey: `${s.programa.id}:${s.aspecto.id}:base` }); _refresh();
    Router.toast('Aspecto por verificar eliminado');
  }
  function evaluarCriterio(index, valor) { const s = _state(); if (!s || !Array.isArray(s.aspecto.criterios_extra) || !s.aspecto.criterios_extra[index]) return; s.aspecto.criterios_extra[index].criterio = valor; Store.upsertInspeccion(s.inspeccion); Scores.calcular(s.inspeccion); Hallazgos.actualizar(s.inspeccion); _programarBorrador(s, { force: true, aspectKey: `${s.programa.id}:${s.aspecto.id}:extra:${index}`, cursor: { programa_id: s.programa.id, aspecto_id: s.aspecto.id, criterio: index } }); _refresh(); }
  function guardarCriterio(index, campo, valor) { const s = _state(); if (!s || !Array.isArray(s.aspecto.criterios_extra) || !s.aspecto.criterios_extra[index]) return; const c = s.aspecto.criterios_extra[index]; c[campo] = valor; if (campo === 'hallazgo') c.obs = valor; Store.upsertInspeccion(s.inspeccion); Hallazgos.actualizar(s.inspeccion); _programarBorrador(s, { aspectKey: `${s.programa.id}:${s.aspecto.id}:extra:${index}`, cursor: { programa_id: s.programa.id, aspecto_id: s.aspecto.id, criterio: index } }); }
  function toggleNorma() { normaAbierta = !normaAbierta; _refresh(); }
  function seleccionarPrograma(i) { const s = _state(); if (s) _programarBorrador(s, { flushOnExit: true, aspectKey: `${s.programa.id}:${s.aspecto.id}:base` }); normaAbierta = false; Store.setUI({ programaIdx: i, aspectoIdx: 0 }); _refresh(); }
  function navegar(dir) { const s = _state(); if (!s) return; _programarBorrador(s, { flushOnExit: true, aspectKey: `${s.programa.id}:${s.aspecto.id}:base` }); let p = s.programaIdx, a = s.aspectoIdx + dir; if (a < 0 && p > 0) { p--; a = s.inspeccion.programas[p].aspectos.length - 1; } else if (a >= s.programa.aspectos.length && p < s.inspeccion.programas.length - 1) { p++; a = 0; } else if (a >= s.programa.aspectos.length) { Router.go('dashboard'); return; } normaAbierta = false; Store.setUI({ programaIdx: p, aspectoIdx: a }); _refresh(); }
