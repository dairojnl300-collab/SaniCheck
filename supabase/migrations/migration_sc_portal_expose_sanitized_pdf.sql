-- Entrega al portal únicamente el acta ya sanitizada y resuelta por código.
-- El portal la muestra en un iframe/ventana separada; no recibe claves.
CREATE OR REPLACE FUNCTION public.sc_portal_get_estado(p_codigo_portal text,p_ip_cliente text,p_gate text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE v_codigo text:=upper(trim(coalesce(p_codigo_portal,''))); v_ip text:=left(coalesce(nullif(btrim(p_ip_cliente),''),sc_client_ip()),64); v_informe sc_informes%rowtype; v_hallazgos jsonb;
BEGIN
