-- SaniCheck — respaldo incremental del progreso de inspecciones.
-- No modifica sc_usuarios ni sc_login_usuario.

ALTER TABLE public.sc_informes
  ADD COLUMN IF NOT EXISTS estado_parcial jsonb,
  ADD COLUMN IF NOT EXISTS estado_parcial_actualizado_en timestamptz;

CREATE INDEX IF NOT EXISTS sc_informes_borradores_idx
  ON public.sc_informes (tecnico_id, estado_parcial_actualizado_en DESC)
  WHERE estado_parcial IS NOT NULL;

COMMENT ON COLUMN public.sc_informes.estado_parcial IS
  'Snapshot liviano del progreso de una inspección en curso; NULL cuando el informe está cerrado. No contiene fotografías.';
COMMENT ON COLUMN public.sc_informes.estado_parcial_actualizado_en IS
  'Momento del último guardado incremental del borrador remoto.';
...
CREATE OR REPLACE FUNCTION public.sc_list_borradores(p_codigo text)
RETURNS TABLE(
  id uuid,
  local_id text,
...
BEGIN
  ...
  RETURN QUERY
  SELECT i.id, i.local_id, i.establecimiento, i.fecha, i.numero_acta,
...
CREATE OR REPLACE FUNCTION public.sc_get_borrador(p_id uuid, p_codigo text)
RETURNS TABLE(
  id uuid,
  local_id text,
  establecimiento jsonb,
...
BEGIN
  ...
  RETURN QUERY
  SELECT i.id, i.local_id, i.establecimiento, i.fecha, i.numero_acta,
         i.estado_parcial, i.creado_en, i.actualizado_en,
         i.estado_parcial_actualizado_en
    FROM public.sc_informes i
   WHERE i.id = p_id
...
-- Los listados de informes finales no deben mostrar filas que aún tienen
-- estado_parcial; el borrador se consulta solo mediante las RPC anteriores.
...
BEGIN
  ...
  RETURN QUERY SELECT i.id, i.local_id, i.tecnico_id, u.nombre, i.establecimiento, i.fecha, i.numero_acta,
   ...
   WHERE i.estado_parcial IS NULL
   ORDER BY i.creado_en DESC;
END;
$function$;
...
REVOKE ALL ON FUNCTION public.sc_guardar_borrador(text,jsonb,date,text,text,jsonb) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.sc_list_borradores(text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.sc_get_borrador(uuid,text) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.sc_guardar_borrador(text,jsonb,date,text,text,jsonb) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.sc_list_borradores(text) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.sc_get_borrador(uuid,text) TO anon, authenticated;
